                  ...
                  <Card className="bg-amber-100/50 border-2 border-amber-300 rounded-xl transition-all duration-300  hover:shadow-lg">
                    <CardContent className="p-4">
                      <div className="flex items-start gap-3 mb-3">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center justify-between mb-1">
                            ...
                            <Badge variant="outline" className="border-amber-600 text-amber-950 bg-amber-100 text-xs font-medium">
                              {(typeof crystal?.priority === 'string' ? crystal.priority : '—').toUpperCase()}
                            </Badge>
                          </div>
                          <p className="text-amber-950 text-sm font-medium">{crystal?.reason ?? '—'}</p>
                        </div>
                      </div>

                      {crystalData.chakraAssociation && crystalData.chakraAssociation.length > 0 && (
                        <div className="mb-3">
                          <p className="text-amber-950 text-xs font-semibold mb-1">Chakra Associations:</p>
                          <div className="flex flex-wrap gap-1">
                            {crystalData.chakraAssociation.slice(0, 3).map((chakra, chakraIndex) => (
                              <Badge
                                key={chakraIndex}
                                variant="outline"
                                className="border-amber-600 text-amber-950 bg-amber-100 text-xs font-medium"
                              >
                                {chakra}
                              </Badge>
                            ))}
                      ...
                      {crystalData.properties && crystalData.properties.length > 0 && (
                        <div className="mb-3">
                          <p className="text-amber-950 text-xs font-semibold mb-1">Properties:</p>
                          <div className="flex flex-wrap gap-1">
                      ...
                      {Array.isArray(crystal?.usage) && crystal.usage.length > 0 && (
                        <div className="mt-3 pt-3 border-t border-amber-300">
                          <p className="text-amber-950 text-xs font-semibold mb-2">How to Use:</p>
                          <ul className="space-y-1">
                            {crystal.usage.slice(0, 3).map((usage, usageIndex) => (
                              <li key={usageIndex} className="text-amber-950 text-xs font-medium flex items-start gap-2">
