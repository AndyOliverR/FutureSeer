...
interface KabbalisticAstrologySeerChatInterfaceProps {
  userId: string;
  userProfile: any;
  westernChartData?: any;
  kabbalisticReport?: Record<string, unknown> | { comprehensiveAnalysis?: Record<string, unknown> };
  sessionId?: string;
}
...
const KABBALISTIC_STARTER_QUESTIONS = [
  'What is my soul correction in this lifetime?',
  'Why do I repeat certain relationship patterns?',
  'What inner quality must I strengthen?',
  'How can I align with my spiritual blueprint?',
];

const SEE_MORE_THRESHOLD = 320;
const PREVIEW_LENGTH = 320;

export default function KabbalisticAstrologySeerChatInterface({
  userId,
  userProfile,
  westernChartData,
  kabbalisticReport,
  sessionId,
}: KabbalisticAstrologySeerChatInterfaceProps) {
  ...
  const sendMessage = async (questionText?: string) => {
    ...
    try {
      const response = await fetchWithFirebaseAuthRequired('/api/ask-kabbalistic-astrology-seer', {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          kabbalisticReport: kabbalisticReport || undefined,
          sessionId,
        }),
      });
      ...
      devLog.error('Kabbalistic Astrology Seer error', error, 'KabbalisticAstrologySeerChatInterface');
      setStreamingMessageId(null);
      setMessages((prev) =>
        prev.map((msg) =>
          msg.id === aiMessageId
      ...
      >
        <div className="max-w-[80%] rounded-xl p-4 bg-gradient-to-br from-amber-50 to-yellow-50 border-2 border-amber-200 text-slate-700">
          <div className="whitespace-pre-wrap leading-relaxed">
            {isStreaming ? (
              <SlowRevealText content={displayContent} minThinkingMs={2000} delayPerWord={85} thinkingLabel="Reflecting on your spiritual blueprint..." className="text-slate-700" />
            ) : (
              displayContent
            )}
          </div>
    ...
    <Card className="flex flex-col h-full bg-gradient-to-br from-amber-50 to-yellow-50 border-2 border-amber-200 shadow-lg transition-all duration-300 min-h-[50vh] max-h-[85vh] overflow-hidden">
      <CardHeader className="border-b border-amber-200 bg-white/80 flex flex-row items-center justify-between gap-2 shrink-0">
        <CardTitle className="text-amber-900 flex items-center gap-2">
          ...
          Kabbalistic Astrology — Ask the Seer
        </CardTitle>
        {messages.length > 0 && (
          <Button
            type="button"
          ...
          {messages.length === 0 && !isLoading ? (
            <div className="text-center py-8">
              ...
              <p className="text-amber-900 font-medium mb-2">
                Ask me anything about your spiritual blueprint and karmic correction…
              </p>
              <p className="text-slate-700 text-sm mt-1 mb-2">
                I'll interpret your chart through Kabbalistic principles to reveal soul tikkun
                (correction), karmic lessons, and spiritual growth patterns.
              </p>
              ...
              <ul className="text-slate-700 text-sm text-left max-w-md mx-auto mb-4 space-y-0.5 list-disc list-inside">
                <li>Your soul correction (Tikkun) in this lifetime</li>
                <li>Karmic patterns and why challenges repeat</li>
                <li>Traits to develop and weaknesses to refine</li>
                <li>Aligning with your spiritual blueprint</li>
              </ul>
              ...
              <p className="text-slate-600 text-xs mt-4">
                Spiritual blueprint and correction only; no event or timing predictions.
              </p>
            </div>
          ) : (
                ...
                <div className="flex justify-start">
                  <div className="bg-gradient-to-br from-amber-50 to-yellow-50 border-2 border-amber-200 rounded-xl p-4">
                    <div className="flex items-center gap-2 text-slate-700">
                      ...
                      Reflecting on your spiritual blueprint...
                    </div>
                  </div>
                </div>
              )}
        ...
        <div className="shrink-0 border-t border-[var(--m3-outline-variant)] bg-[var(--m3-surface-container)] p-4">
          ...
          >
            <Input
              ...
              placeholder="Ask about Tikkun, karmic patterns, or spiritual correction..."
              disabled={isLoading}
