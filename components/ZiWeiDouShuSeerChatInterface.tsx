...
const ZIWEI_STARTER_QUESTIONS = [
  'What does my Life Palace say about my core personality?',
  'How is my wealth and career indicated in my chart?',
  'What does my current 10-year luck (大限) suggest?',
  'How can I use the Four Transformations in my decisions?',
];

const SEE_MORE_THRESHOLD = 320;
const PREVIEW_LENGTH = 320;

export default function ZiWeiDouShuSeerChatInterface({
  userId,
  userProfile,
  ziweiReport,
  sessionId,
}: ZiWeiDouShuSeerChatInterfaceProps) {
  ...
  const sendMessage = async (questionText?: string) => {
    ...
    try {
      ...
      devLog.error('Zi Wei Dou Shu Seer error', error, 'ZiWeiDouShuSeerChatInterface');
      setStreamingMessageId(null);
      setMessages((prev) =>
        prev.map((msg) =>
          msg.id === aiMessageId
      ...
      >
        <div className="max-w-[80%] rounded-xl p-4 bg-gradient-to-br from-amber-50 to-yellow-50 border-2 border-amber-200 text-slate-700">
          <div className="whitespace-pre-wrap leading-relaxed">
            {isStreaming ? (
              <SlowRevealText content={displayContent} minThinkingMs={2000} delayPerWord={85} thinkingLabel="Reflecting on your Purple Star chart..." className="text-slate-700" />
            ) : (
              displayContent
            )}
          </div>
          ...
          {messages.length === 0 && !isLoading ? (
            <div className="text-center py-8">
              ...
              <p className="text-amber-900 font-medium mb-2">
                Ask me anything about your Zi Wei Dou Shu chart and report…
              </p>
              <p className="text-slate-700 text-sm mt-1 mb-2">
                I'll interpret your 12 palaces, major stars, Four Transformations, and luck cycles
                based on your generated report.
              </p>
              ...
              <ul className="text-slate-700 text-sm text-left max-w-md mx-auto mb-4 space-y-0.5 list-disc list-inside">
                <li>Life Palace and core personality</li>
                <li>Wealth and career timing</li>
                <li>Relationship and spouse palace</li>
                <li>10-year luck (大限) and annual flow (流年)</li>
              </ul>
              ...
              {!ziweiReport && (
                <p className="text-slate-600 text-xs mt-4">
                  Generate your mystical profile for a personalized reading based on your chart.
                </p>
              )}
            </div>
          ) : (
                ...
                <div className="flex justify-start">
                  <div className="bg-gradient-to-br from-amber-50 to-yellow-50 border-2 border-amber-200 rounded-xl p-4">
                    <div className="flex items-center gap-2 text-slate-700">
                      ...
                      Reflecting on your Purple Star chart...
                    </div>
                  </div>
                </div>
              )}
        ...
        <div className="shrink-0 border-t border-amber-200 bg-white/80 p-4">
          ...
          >
            <Input
              ...
              placeholder="Ask about your palaces, stars, 大限, or 四化..."
              disabled={isLoading}
