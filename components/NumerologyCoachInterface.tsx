...
export function NumerologyCoachInterface() {
  ...
  const [suggestedQuestions] = useState([
    "What does my Life Path Number reveal about my purpose?",
    "How can I use my Destiny Number in my career?",
    "What are my biggest strengths according to numerology?",
    "How can I work through my karmic debts?",
    "What does my Personal Year Number mean for me?",
    "How can I improve my relationships using numerology?",
    "What career paths align with my numbers?",
    "How can I develop my master numbers?"
  ])
  ...
  const handleSendMessage = async () => {
    if (!inputValue.trim() || !numerologyData || isLoading) return

    const userMessage: NumerologyMessage = {
    ...
    try {
      const coachingResponse = await getNumerologyCoaching(
        numerologyData.userId,
        numerologyData,
        inputValue.trim()
      )

      const coachMessage: NumerologyMessage = {
      ...
      const errorMessage: NumerologyMessage = {
        id: (Date.now() + 1).toString(),
        type: 'coach',
        content: "I'm having trouble accessing your numerology insights right now. Please try again in a moment.",
        timestamp: new Date()
      }
      ...
      <Card className="bg-slate-800/50 border-slate-600">
        <CardContent className="p-6">
          <div className="flex items-center justify-center">
            ...
            <span className="ml-3 text-slate-300">Loading your numerology profile...</span>
          </div>
        </CardContent>
      </Card>
    )
  ...
  if (error || !numerologyData) {
    return (
      <Card className="bg-slate-800/50 border-slate-600">
        <CardContent className="p-6 text-center">
          <Hash className="w-12 h-12 text-purple-400 mx-auto mb-4" />
          <h3 className="text-lg font-semibold text-white mb-2">Numerology Profile Required</h3>
          <p className="text-slate-400 mb-4">
            {error || 'Please complete your profile with full name and birth date to access numerology coaching.'}
          </p>
      ...
      <Card className="bg-slate-800/50 border-slate-600">
        <CardHeader>
          <CardTitle className="text-purple-400 flex items-center gap-2">
            ...
            Your Numerology Coach
          </CardTitle>
          <p className="text-sm text-slate-400">
            Ask me anything about your numbers, and I'll provide personalized guidance based on your unique numerological profile.
          </p>
        </CardHeader>
      </Card>
          ...
          <ScrollArea className="flex-1 p-4">
            {messages.length === 0 ? (
              <div className="text-center py-8">
                ...
                <h3 className="text-lg font-semibold text-white mb-2">Start Your Numerology Session</h3>
                <p className="text-slate-400 mb-6">
                  I'm here to provide personalized guidance based on your numerological profile.
                </p>
                    ...
                    <div className="bg-slate-700 text-slate-200 rounded-lg p-3 max-w-[80%]">
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex items-center gap-1">
                          <div className="animate-pulse">Analyzing your numbers</div>
          ...
          <div className="border-t border-slate-600 p-4">
            <div className="flex gap-2">
              <Input
                ...
                placeholder="Ask me about your Life Path, Destiny Number, or any numerology topic..."
                className="flex-1 bg-slate-700 border-slate-600 text-white placeholder:text-slate-400 focus:border-purple-500"
                disabled={isLoading || !numerologyData}
              />
              <Button
                onClick={handleSendMessage}
                disabled={!inputValue.trim() || isLoading || !numerologyData}
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <Badge variant="outline" className="border-blue-500 text-blue-400">
                Destiny {numerologyData.destinyNumber}
              </Badge>
              <Badge variant="outline" className="border-pink-500 text-pink-400">
                Soul {numerologyData.soulNumber}
              </Badge>
            </div>
          </div>
        </CardContent>
