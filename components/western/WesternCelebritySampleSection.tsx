      ...
      <CardContent className="px-3 pb-4 pt-0 sm:px-6 sm:pb-6">
        <ul className="divide-y divide-slate-200 rounded-2xl border border-slate-200 bg-white overflow-hidden">
          {samples.map((c, i) => (
            <li
              key={`${c.name}-${i}`}
              className="flex flex-col sm:flex-row sm:items-baseline sm:justify-between gap-1 sm:gap-2 px-3 sm:px-4 py-3 text-sm min-w-0"
            >
              <div className="flex items-center justify-between gap-2 min-w-0">
                <span className="font-medium text-slate-900 truncate">{c.name}</span>
                <span className="text-amber-800 font-medium shrink-0">{c.sunSign}</span>
              </div>
