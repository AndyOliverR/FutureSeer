...
interface SynastrySeerChatInterfaceProps {
  analysis: SynastryCompatibility | null;
  userId?: string;
}
...
const EMPTY_STATE_MESSAGE =
  'Run Synastry analysis for both people first to use Ask the Seer.';
...
export function SynastrySeerChatInterface({
  analysis,
  userId,
}: SynastrySeerChatInterfaceProps) {
  const [messages, setMessages] = useState<SynastryMessage[]>([]);
  ...
  const hasAnalysis =
    !!analysis && !!analysis.person1Natal && !!analysis.person2Natal;
  ...
  const sendMessage = async (questionText?: string) => {
    ...
    const userMessage: SynastryMessage = {
      id: `user_${Date.now()}`,
      type: 'user',
    ...
    const aiMessage: SynastryMessage = {
      id: aiMessageId,
      type: 'seer',
      content: '',
      timestamp: Date.now(),
    ...
    try {
      const response = await fetchWithFirebaseAuthRequired('/api/ask-synastry-seer', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          userId: userId ?? '',
          question: messageToSend,
          synastryAnalysis: analysis,
        }),
      });
      ...
      if (!response.ok) {
        ...
        if (response.status === 400) {
          ...
          if (
            ...
            errorMessage.toLowerCase().includes('run')
          ) {
            setMessages((prev) =>
              prev.map((msg) =>
                msg.id === aiMessageId
        ...
        setMessages((prev) =>
          prev.map((msg) =>
            msg.id === aiMessageId
              ? {
                  ...msg,
                  content: 'Synastry Seer is unavailable. Please try again.',
                }
              : msg
          )
        );
  ...
  const formatMessage = (message: SynastryMessage) => {
                ...
                <div className="flex justify-start">
                  <div className="bg-gradient-to-br from-amber-50 to-yellow-50 border-2 border-amber-200 rounded-xl p-4">
                    <div className="flex items-center gap-2 text-slate-700">
                      ...
                      Consulting your charts and aspects…
                    </div>
                  </div>
                </div>
              )}
