...
const MUNDANE_HINT_QUESTIONS = [
  'What themes define this year globally?',
  'What pressures affect governments now?',
  'Is this a stabilizing or volatile period?',
  'Which sectors face disruption?',
];

interface MundaneSeerChatProps {
  userProfile?: any;
  analysis?: any;
}

export function MundaneSeerChat({ userProfile, analysis }: MundaneSeerChatProps) {
  ...
  const sendMessage = async (questionText?: string) => {
    ...
    try {
      const response = await fetch('/api/chat/mundane-seer', {
        method: 'POST',
        ...
        <CardHeader className="border-b border-amber-200 bg-white/80 shrink-0 flex flex-row items-center justify-between gap-2">
          <div>
            <CardTitle className="flex items-center gap-2 text-amber-900">
              ...
              Ask the Seer — Mundane Astrology
            </CardTitle>
            <p className="text-sm text-amber-800 mt-1">
              Collective conditions and pressures, not specific outcomes.
            </p>
          </div>
          {messages.length > 0 && (
            <Button
        ...
        <CardContent className="pt-6 flex flex-col min-h-0">
            ...
            {messages.length === 0 && !isLoading ? (
              <div className="text-center py-8">
                ...
                <p className="text-amber-900 font-medium mb-2">
                  Ask about themes, pressures, stability, or sectors…
                </p>
                <div className="flex flex-wrap gap-2 justify-center mt-4">
                  {MUNDANE_HINT_QUESTIONS.map((q, i) => (
                    <Button
                      key={i}
                      variant="outline"
                      size="sm"
          ...
          >
            <Input
              ...
              placeholder="Ask about themes, pressures, stability, or sectors…"
