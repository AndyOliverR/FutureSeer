...
interface RunesSeerChatInterfaceProps {
  reading: RuneReading | null | undefined;
  userId?: string;
  userProfile?: any;
  sessionId?: string;
}
...
export default function RunesSeerChatInterface({
  ...
  userProfile,
  sessionId,
}: RunesSeerChatInterfaceProps) {
  const [messages, setMessages] = useState<RunesSeerMessage[]>([]);
  ...
  const hasRunes = Array.isArray(reading?.runes) && reading.runes.length > 0;
  const hasReading = !!reading && hasRunes;
  ...
  const sendMessage = async (questionText?: string) => {
    ...
    if (!messageToSend || isLoading || !hasReading || !reading) return;

    const userMessage: RunesSeerMessage = {
      id: `user_${Date.now()}`,
      type: 'user',
    ...
    const aiMessage: RunesSeerMessage = {
      id: aiMessageId,
      type: 'seer',
      content: '',
      timestamp: Date.now(),
    ...
    const body = {
      ...
      runeReading: reading,
      sessionId: sessionId ?? `runes_${Date.now()}`,
    };
  ...
  const formatMessage = (message: RunesSeerMessage) => {
      ...
      <Card className="flex flex-col h-full bg-gradient-to-br from-amber-50 to-yellow-50 border-2 border-amber-200 min-h-[50vh] max-h-[85vh] overflow-hidden">
        ...
        <CardContent className="flex-1 flex flex-col items-center justify-center p-8">
          ...
          <p className="text-sm text-amber-700 mt-2 text-center">
            Perform a rune reading in the Reading tab, then return here to ask about the cast runes.
          </p>
        </CardContent>
      </Card>
    );
