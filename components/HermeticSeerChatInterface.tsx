...
interface HermeticSeerChatInterfaceProps {
  userId: string;
  userProfile: any;
  westernChartData?: any;
  hermeticReport?: Record<string, unknown> | { comprehensiveAnalysis?: Record<string, unknown> };
  sessionId?: string;
}
...
const HERMETIC_STARTER_QUESTIONS = [
  'What does Hermetic astrology reveal about my inner balance?',
  'What alchemical lesson am I working through?',
  'Which element dominates my personality?',
  'Where do I need internal transformation?',
];

const SEE_MORE_THRESHOLD = 320;
const PREVIEW_LENGTH = 320;

export default function HermeticSeerChatInterface({
  userId,
  userProfile,
  westernChartData,
  hermeticReport,
  sessionId,
}: HermeticSeerChatInterfaceProps) {
  ...
  const sendMessage = async (questionText?: string) => {
    ...
    try {
      const response = await fetchWithFirebaseAuthRequired('/api/ask-hermetic-seer', {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          hermeticReport: hermeticReport || undefined,
          sessionId,
        }),
      });
      ...
      devLog.error('Hermetic Seer error', error, 'HermeticSeerChatInterface');
      setStreamingMessageId(null);
      setMessages((prev) =>
        prev.map((msg) =>
          msg.id === aiMessageId
  ...
  return (
    <Card className="flex flex-col h-full bg-gradient-to-br from-teal-50 to-emerald-50 border-2 border-teal-200 shadow-lg transition-all duration-300 min-h-[50vh] max-h-[85vh] overflow-hidden">
      <CardHeader className="border-b border-teal-200 bg-white/80 flex flex-row items-center justify-between gap-2 shrink-0">
        <CardTitle className="text-teal-900 flex items-center gap-2">
          ...
          Hermetic Astrology — Ask the Seer
        </CardTitle>
        {messages.length > 0 && (
          <Button
            type="button"
          ...
          {messages.length === 0 && !isLoading ? (
            <div className="text-center py-8">
              ...
              <p className="text-teal-900 font-medium mb-2">
                Ask me anything about your spiritual mechanics and inner alchemy…
              </p>
              <p className="text-slate-700 text-sm mt-1 mb-2">
                I'll interpret your chart through Hermetic principles to reveal archetypal forces,
                polarity balance, and inner transformation dynamics.
              </p>
              ...
              <ul className="text-slate-700 text-sm text-left max-w-md mx-auto mb-4 space-y-0.5 list-disc list-inside">
                <li>Energetic imbalance and elemental balance</li>
                <li>What you must transmute or refine</li>
                <li>How your planetary energies interact</li>
                <li>Where you resist transformation</li>
              </ul>
              <div className="flex flex-wrap gap-2 justify-center mt-4">
                {HERMETIC_STARTER_QUESTIONS.map((q, i) => (
                  <Button
                    key={i}
                    variant="outline"
                    size="sm"
              ...
              <p className="text-slate-600 text-xs mt-4">
                Spiritual mechanics only; no event or timing predictions.
              </p>
            </div>
          ) : (
                ...
                <div className="flex justify-start">
                  <div className="bg-gradient-to-br from-teal-50 to-emerald-50 border-2 border-teal-200 rounded-xl p-4">
                    <div className="flex items-center gap-2 text-slate-700">
                      ...
                      Reflecting on your spiritual mechanics...
                    </div>
                  </div>
                </div>
              )}
        ...
        <div className="shrink-0 border-t border-teal-200 bg-white/80 p-4">
          ...
          >
            <Input
              ...
              placeholder="Ask about energy balance, inner alchemy, or polarity..."
              disabled={isLoading}
