...
export interface FengShuiLayoutInput {
  main_door?: string;
  bedroom?: string;
  kitchen?: string;
  toilet?: string;
...
const EMPTY_STATE_MESSAGE =
  'Complete your profile and generate Feng Shui analysis first to use Ask the Seer.';
...
export default function FengShuiSeerChatInterface({
  ...
  facingDirection,
  layout,
}: FengShuiSeerChatInterfaceProps) {
  const [messages, setMessages] = useState<FengShuiSeerMessage[]>([]);
  ...
  const hasKua = analysis?.kua != null;
  const hasAnalysis = !!analysis && hasKua;
  ...
  const sendMessage = async (questionText?: string) => {
    ...
    const userMessage: FengShuiSeerMessage = {
      id: `user_${Date.now()}`,
      type: 'user',
    ...
    const aiMessage: FengShuiSeerMessage = {
      id: aiMessageId,
      type: 'seer',
      content: '',
      timestamp: Date.now(),
    ...
    const body = {
      ...
      fengShuiAnalysis: analysis,
      sessionId: sessionId ?? `feng-shui_${Date.now()}`,
      ...(facingDirection?.trim() && { facing_direction: facingDirection.trim() }),
      ...(layout && { layout }),
    };

    const performFetch = () =>
      fetchWithFirebaseAuthRequired('/api/ask-feng-shui-seer', {
        method: 'POST',
  ...
  const formatMessage = (message: FengShuiSeerMessage) => {
