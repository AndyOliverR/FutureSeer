...
import type { AkashicReading } from '@/lib/akashicRecordsIntelligence';
...
const AKASHIC_HINT_QUESTIONS = [
  'What theme is central to my life right now?',
  'Why does this pattern keep repeating?',
  'What aspect of myself wants integration?',
  'What perspective helps me move forward?',
];

interface AkashicSeerChatInterfaceProps {
  reading: AkashicReading;
  userProfile?: any;
}

export function AkashicSeerChatInterface({ reading, userProfile }: AkashicSeerChatInterfaceProps) {
        ...
        <CardHeader className="border-b border-amber-200 bg-white/80 shrink-0 flex flex-row items-center justify-between gap-2">
          <div>
            <CardTitle className="flex items-center gap-2 text-amber-900">
              ...
              Ask the Seer — Akashic Records
            </CardTitle>
            <p className="text-sm text-amber-800 mt-1">
              Symbolic insight into life themes, not factual history or fate.
            </p>
          </div>
          {messages.length > 0 && (
            <Button
        ...
        <CardContent className="pt-6 flex flex-col min-h-0">
            ...
            {messages.length === 0 && !isLoading ? (
              <div className="text-center py-8">
                ...
                <p className="text-amber-900 font-medium mb-2">
                  Ask about themes, patterns, purpose, or perspective…
                </p>
                <div className="flex flex-wrap gap-2 justify-center mt-4">
                  {AKASHIC_HINT_QUESTIONS.map((q, i) => (
                    <Button
                      key={i}
                      variant="outline"
                      size="sm"
          ...
          >
            <Input
              ...
              placeholder="Ask about themes, patterns, purpose, or perspective…"
