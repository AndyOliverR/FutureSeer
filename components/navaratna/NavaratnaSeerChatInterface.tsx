...
interface NavaratnaSeerChatInterfaceProps {
  userId?: string;
  userProfile?: any;
...
export default function NavaratnaSeerChatInterface({
  ...
  sessionId,
}: NavaratnaSeerChatInterfaceProps) {
  const analysis = analysisProp ?? navaratnaAnalysis;
  const [messages, setMessages] = useState<NavaratnaMessage[]>([]);
  ...
  const hasAnalysis = !!analysis && !!analysis.chartSummary;
  ...
  const sendMessage = async (questionText?: string) => {
    ...
    const userMessage: NavaratnaMessage = {
      id: `user_${Date.now()}`,
      type: 'user',
    ...
    const aiMessage: NavaratnaMessage = {
      id: aiMessageId,
      type: 'seer',
      content: '',
      timestamp: Date.now(),
    ...
    try {
      const response = await fetch('/api/ask-navaratna-seer', {
        ...
        body: JSON.stringify({
          ...
          navaratnaAnalysis: analysis,
          sessionId: sessionId ?? `navaratna_${analysis.userId || Date.now()}`,
        }),
      });
      ...
      if (!response.ok) {
        ...
        setMessages((prev) =>
          prev.map((msg) =>
            msg.id === aiMessageId
              ? { ...msg, content: 'Navaratna Seer is unavailable. Please try again.' }
              : msg
          )
        );
        return;
  ...
  const formatMessage = (message: NavaratnaMessage) => {
