...
interface TarotSeerChatInterfaceProps {
  userId: string
  userProfile: any
...
export default function TarotSeerChatInterface({
  userId,
  userProfile,
  tarotProfileData,
  ...
  currentReading,
  sessionId
}: TarotSeerChatInterfaceProps) {
  const [messages, setMessages] = useState<Message[]>([])
  const [question, setQuestion] = useState('')
  const [isLoading, setIsLoading] = useState(false)
  const [currentSessionId, setCurrentSessionId] = useState(sessionId || `tarot-seer-${Date.now()}`)
  ...
  const sendMessage = async (questionText?: string) => {
    ...
    try {
      const response = await fetchWithFirebaseAuthRequired('/api/ask-tarot-seer', {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          numerologyData: numerologyData || combinedSystemData?.numerology,
          combinedSystemData,
