...
function WesternWheel({ chart }: { chart: UnifiedChartData }) {
  ...
  return (
    <svg viewBox={`0 0 ${size} ${size}`} className="w-full h-auto">
      ...
      <circle cx={center} cy={center} r={220} fill="none" stroke={tokens.ringStroke} strokeWidth="1.5" />
      <circle cx={center} cy={center} r={165} fill="none" stroke={tokens.ringStroke} strokeWidth="1" />
      {Array.from({ length: 12 }, (_, i) => {
        ...
        return (
          <g key={`z-${a}`}>
            <line x1={center} y1={center} x2={p.x} y2={p.y} stroke={tokens.ringStroke} strokeWidth="0.6" opacity="0.35" />
            <text x={t.x} y={t.y} textAnchor="middle" dominantBaseline="middle" fontSize="16" fill={tokens.textPrimary}>{symbol}</text>
          </g>
        );
      })}
      {chart.aspects?.map((aspect, i) => {
        ...
        return <line key={`a-${i}`} x1={a.x} y1={a.y} x2={b.x} y2={b.y} stroke={tokens.accent} strokeWidth="0.8" opacity="0.5" />;
      })}
      {chart.points.map((point) => {
        const p = polar(center, 145, point.longitude);
        return (
          <g key={point.id}>
            <circle cx={p.x} cy={p.y} r={11} fill={tokens.background} stroke={tokens.ringStroke} strokeWidth="1" />
            <text x={p.x} y={p.y} textAnchor="middle" dominantBaseline="middle" fontSize="10" fill={tokens.textPrimary}>{point.shortLabel ?? point.label.slice(0, 2)}</text>
          </g>
        );
      })}
    </svg>
  ...
  return (
    <svg viewBox={`0 0 ${size} ${size}`} className="w-full h-auto">
      ...
      {Array.from({ length: 16 }, (_, i) => {
        ...
        const col = i % 4;
        return <rect key={i} x={col * box} y={row * box} width={box} height={box} fill="none" stroke={tokens.ringStroke} strokeWidth="1" />;
      })}
      {chart.houses.map((house) => {
        ...
        return (
          <g key={`h-${house.number}`}>
            <text x={x + 8} y={y + 14} fontSize="11" fill={tokens.textSecondary}>{house.number}</text>
            <text x={x + box / 2} y={y + 22} textAnchor="middle" fontSize="14" fill={tokens.textPrimary}>{ZODIAC_SYMBOLS[(house.number - 1 + 12) % 12]}</text>
            {housePoints.slice(0, 4).map((p, idx) => (
              <text key={p.id} x={x + box / 2} y={y + 40 + idx * 16} textAnchor="middle" fontSize="11" fill={tokens.textPrimary}>
...
function NakshatraWheel({ chart }: { chart: UnifiedChartData }) {
  ...
  return (
    <svg viewBox={`0 0 ${size} ${size}`} className="w-full h-auto">
      <rect width={size} height={size} fill={tokens.background} />
      <circle cx={center} cy={center} r={225} fill="none" stroke={tokens.ringStroke} strokeWidth="1.2" />
      <circle cx={center} cy={center} r={170} fill="none" stroke={tokens.ringStroke} strokeWidth="1" />
      <circle cx={center} cy={center} r={120} fill="none" stroke={tokens.ringStroke} strokeWidth="0.9" />
      {Array.from({ length: 27 }, (_, i) => {
        ...
        return (
          <g key={i}>
            <line x1={p1.x} y1={p1.y} x2={p2.x} y2={p2.y} stroke={tokens.ringStroke} strokeWidth="0.45" opacity="0.55" />
            <text x={t.x} y={t.y} textAnchor="middle" dominantBaseline="middle" fontSize="8.5" fill={tokens.textSecondary}>
              {NAKSHATRAS[i]}
            </text>
          </g>
        );
      ...
      {chart.points.map((point, idx) => {
        ...
        return (
          <text key={point.id} x={p.x} y={p.y} textAnchor="middle" dominantBaseline="middle" fontSize="12" fill={tokens.textPrimary}>
