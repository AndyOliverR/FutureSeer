...
import type { OghamReport } from '@/lib/ogham/oghamReportGenerator';
...
const OGHAM_HINT_QUESTIONS = [
  'What phase is this situation in?',
  'What supports growth right now?',
  'Is it time to begin, wait, or consolidate?',
  'What action suits this phase?',
];

interface OghamSeerChatInterfaceProps {
  report: OghamReport;
  userProfile?: any;
  userId?: string;
}

export function OghamSeerChatInterface({
  report,
  userProfile,
  userId,
}: OghamSeerChatInterfaceProps) {
  ...
  const sendMessage = async (questionText?: string) => {
    ...
    if (!questionToSend || isLoading || !report) return;
    ...
    try {
      ...
      devLog.error('Error calling Ogham Seer API:', error, 'OghamSeerChatInterface');
      setMessages((prev) =>
        prev.map((msg) =>
          msg.id === assistantId
            ? {
        ...
        <CardHeader className="border-b border-amber-200 bg-white/80 shrink-0 flex flex-row items-center justify-between gap-2">
          <div>
            <CardTitle className="flex items-center gap-2 text-amber-900">
              ...
              Ask the Seer — Ogham Divination
            </CardTitle>
            <p className="text-sm text-amber-800 mt-1">
              Growth conditions and phases, not guaranteed outcomes.
            </p>
          </div>
          {messages.length > 0 && (
            <Button
        ...
        <CardContent className="pt-6 flex flex-col min-h-0">
            ...
            {messages.length === 0 && !isLoading ? (
              <div className="text-center py-8">
                ...
                <p className="text-amber-900 font-medium mb-2">
                  Ask about phases, growth, or phase-appropriate action…
                </p>
                <div className="flex flex-wrap gap-2 justify-center mt-4">
                  {OGHAM_HINT_QUESTIONS.map((q, i) => (
                    <Button
                      key={i}
                      variant="outline"
                      size="sm"
          ...
          >
            <Input
              ...
              placeholder="Ask about phases, growth, or phase-appropriate action…"
