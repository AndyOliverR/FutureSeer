...
export const PalmistryDashboardHero = memo(function PalmistryDashboardHero({ analysis, userProfile }: PalmistryDashboardHeroProps) {
  if (!analysis) {
    return (
      <Card className="bg-gradient-to-br from-amber-50 via-orange-50 to-yellow-50 border-2 border-amber-300 shadow-lg rounded-3xl mb-8">
        <CardContent className="p-8">
          <div className="text-center">
            <Hand className="w-16 h-16 text-amber-600 mx-auto mb-4" />
            <h2 className="text-2xl font-bold text-amber-900 mb-2">Palm Reading Analysis</h2>
  ...
  return (
    <motion.div
      ...
      className="mb-8"
    >
      <Card className="bg-gradient-to-br from-amber-50 via-amber-50 to-yellow-50 border-2 border-amber-300 shadow-lg rounded-3xl overflow-hidden">
        {/* Accent Bar */}
        ...
        <CardContent className="p-8">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-1 flex flex-col items-center justify-center">
              {userProfile?.palmPhotoUrl ? (
                <div className="relative w-full aspect-square max-w-[240px] rounded-2xl overflow-hidden border-4 border-amber-400 shadow-xl">
                  <img 
                ...
                <div className="w-48 h-48 flex items-center justify-center bg-gradient-to-br from-amber-100 to-orange-100 rounded-2xl border-4 border-amber-400 shadow-xl">
                  <Hand className="w-24 h-24 text-amber-600" />
                </div>
              )}
            ...
            <div className="lg:col-span-1 flex flex-col justify-center space-y-4">
              <div className="text-center mb-4">
                <h2 className="text-3xl font-bold text-amber-900 mb-2 flex items-center justify-center gap-2">
                  <Sparkles className="w-8 h-8 text-amber-600" />
                  Palm Analysis
                </h2>
                <p className="text-slate-700 text-sm">
                  Energy Level: <span className="font-bold text-amber-700">{analysis.energyScore}/100</span>
                </p>
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div className="bg-white/60 rounded-xl p-3 border border-amber-200 text-center">
                  <div className="text-2xl mb-1">🔥</div>
                  <div className="text-xs text-slate-600 mb-1">Primary Element</div>
                  <div className="text-sm font-bold text-amber-900 capitalize">{analysis.elements?.primary || 'Balanced'}</div>
                </div>
                <div className="bg-white/60 rounded-xl p-3 border border-amber-200 text-center">
                  <div className="text-2xl mb-1">⭐</div>
                  <div className="text-xs text-slate-600 mb-1">Confidence</div>
                  <div className="text-sm font-bold text-amber-900">{analysis.confidenceLevel}%</div>
                </div>
                <div className="bg-white/60 rounded-xl p-3 border border-amber-200 text-center">
                  <div className="text-2xl mb-1">📏</div>
                  <div className="text-xs text-slate-600 mb-1">Major Lines</div>
                  <div className="text-sm font-bold text-amber-900">{analysis.lines?.length || 0}</div>
                </div>
                <div className="bg-white/60 rounded-xl p-3 border border-amber-200 text-center">
                  <div className="text-2xl mb-1">⛰️</div>
                  <div className="text-xs text-slate-600 mb-1">Mounts</div>
                  <div className="text-sm font-bold text-amber-900">{analysis.mounts?.length || 0}</div>
                </div>
              </div>
            </div>
            ...
            <div className="lg:col-span-1 flex flex-col justify-center space-y-3">
              <h3 className="text-lg font-bold text-amber-900 mb-2 text-center">Key Lines</h3>
          ...
          {analysis.timing?.currentPhase && (
            <div className="mt-6 pt-6 border-t border-amber-300">
              <div className="text-center">
                ...
                <p className="text-lg font-bold text-amber-900">{analysis.timing.currentPhase}</p>
              </div>
            </div>
          )}
        </CardContent>
