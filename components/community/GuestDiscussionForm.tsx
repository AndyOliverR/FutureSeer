...
async function getGuestCaptchaToken(): Promise<string> {
...
export function GuestDiscussionForm({ onSubmit, onCancel }: GuestDiscussionFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const captchaToken = await getGuestCaptchaToken();
      await onSubmit({
        title: title.trim(),
      ...
      <Card className="bg-slate-900/95 backdrop-blur-sm border-amber-500/20">
        ...
        <CardContent>
          <p className="text-xs text-amber-200/70 mb-4">
            Verified with reCAPTCHA; strict rate limits apply.{" "}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex gap-3">
              ...
              <Button
                ...
                disabled={
                  submitting || !displayName.trim() || !title.trim() || !content.trim()
                }
              >
