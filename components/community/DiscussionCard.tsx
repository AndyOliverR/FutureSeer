...
export function DiscussionCard({ discussion, onVote, userVote, onClick, onOpenThread }: DiscussionCardProps) {
  const netVotes = discussion.upvotes - discussion.downvotes;

  return (
    <div 
      className="group flex gap-3 sm:gap-4 p-3 sm:p-4 bg-slate-800/50 rounded-lg border border-slate-700 hover:border-amber-500/30 hover:shadow-[0_0_0_1px_rgba(251,191,36,0.15)] transition-all duration-200 cursor-pointer"
      onClick={onClick}
    >
      {/* Voting */}
      <div className="flex flex-col items-center gap-1.5 sm:gap-2">
        ...
        <Button
          ...
          onClick={(e) => {
            e.stopPropagation();
            onOpenThread?.(discussion.id);
          }}
          className="shrink-0 border-purple-400/40 text-purple-200 hover:bg-purple-500/20 active:scale-[0.98]"
        >
          Open thread
        </Button>
      </div>
