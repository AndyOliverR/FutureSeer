...
describe('/api/tools/data auth', () => {
  ...
  async function postAvailableTools(userId: string) {
    const { POST } = await import('@/app/api/tools/data/route');
    const req = new NextRequest('http://localhost:3000/api/tools/data', {
      method: 'POST',
      headers: {
  ...
  it('does not list tools for another user via POST', async () => {
    mockVerifyUserRequest.mockResolvedValueOnce({ ok: true, uid: 'attacker' });
    mockResolveOwnedUserId.mockReturnValueOnce(null);

    const res = await postAvailableTools('victim');
