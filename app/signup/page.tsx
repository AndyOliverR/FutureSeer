        ...
        <div className="w-full bg-surface-container-high rounded-[32px] p-6 sm:p-8 border border-outline-variant shadow-2xl overflow-hidden">
          ...
          {showSignupFlow ? (
            <SignupFlow
              ...
              selectedCountry={selectedCountry} initialPlan={planParam || undefined}
              onComplete={handleSignupFlowComplete} onError={setError}
            />
          ) : (
            <form onSubmit={handleBasicInfoSubmit} className="space-y-5">
              <div className="space-y-2">
                ...
                <div className="relative">
                  <User className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-surface-on-variant" />
                  <Input value={displayName} onChange={e => setDisplayName(e.target.value)} placeholder="Full Name" className="h-14 pl-12 bg-surface-container-low border-outline-variant rounded-2xl font-medium" />
                </div>
              </div>
              ...
              <div className="space-y-2">
                <Label className="text-xs uppercase font-bold text-amber-400 tracking-widest ml-1">Email</Label>
                <div className="relative">
                  <Mail className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-surface-on-variant" />
                  <Input value={email} onChange={e => setEmail(e.target.value)} type="email" placeholder="name@example.com" className="h-14 pl-12 bg-surface-container-low border-outline-variant rounded-2xl font-medium" />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input value={password} onChange={e => setPassword(e.target.value)} type={showPassword ? "text" : "password"} placeholder="Create Password" className="h-14 pl-12 pr-12 bg-surface-container-low border-outline-variant rounded-2xl font-medium" />
              ...
              <div className="space-y-2">
                <Label className="text-xs uppercase font-bold text-amber-400 tracking-widest ml-1">Confirm Password</Label>
                <div className="relative">
                  <Lock className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-surface-on-variant" />
                  <Input value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} type={showConfirmPassword ? "text" : "password"} placeholder="Repeat Password" className="h-14 pl-12 pr-12 bg-surface-container-low border-outline-variant rounded-2xl font-medium" />
              ...
              {referralCode && <p className="text-center text-green-400 text-xs font-bold uppercase tracking-wider">✓ Referral bonus active</p>}

              <Button type="submit" disabled={isLoading} className="w-full h-14 bg-amber-500 hover:bg-amber-600 text-slate-900 rounded-2xl font-bold text-lg shadow-lg active:scale-[0.98] transition-all">
                Next Step
              </Button>
            </form>
          )}
