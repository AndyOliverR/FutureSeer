...
export async function POST(request: NextRequest) {
  try {
    ...
    const __toolSeerGate = await enforceToolSeerGate(request, body, 'ask_esoteric_seer')
    ...
    const { stream } = await callTextStream({ label: 'ask-esoteric-seer', model: 'llama-3.3-70b-versatile',
      userId,
        ...
        async start(controller) {
          try {
            ...
            if (fullResponse.trim()) {
              await cacheToolSeerAnswer('ask-esoteric-seer', userId, question, fullResponse);
            }
          } catch (error) {
