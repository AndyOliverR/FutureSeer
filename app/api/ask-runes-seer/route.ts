...
export async function POST(request: NextRequest) {
  try {
    ...
    const __toolSeerGate = await enforceToolSeerGate(request, body, 'ask_runes_seer')
    ...
    const { stream } = await callTextStream({ label: 'ask-runes-seer', model: GROQ_DEFAULT_TEXT_MODEL,
      userId,
        ...
        async start(controller) {
          try {
            ...
            if (fullResponse.trim()) {
              await cacheToolSeerAnswer('ask-runes-seer', userId, question, fullResponse);
            }
          } catch (error) {
