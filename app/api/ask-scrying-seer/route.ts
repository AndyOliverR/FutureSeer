...
export async function POST(request: NextRequest) {
  try {
    ...
    const userId = await resolveToolSeerUserId(request, body, 'ask_scrying_seer')
    if (!userId) {
...
Answer the user's question in 2–4 short paragraphs, referencing the report where relevant. Keep a calm, reflective tone.`;
    ...
    const { stream } = await callTextStream({ label: 'ask-scrying-seer', model: 'llama-3.3-70b-versatile',
      userId,
        ...
        async start(controller) {
          try {
            ...
            if (fullResponse.trim()) {
              await cacheToolSeerAnswer('ask-scrying-seer', userId, question, fullResponse);
            }
          } catch (error) {
