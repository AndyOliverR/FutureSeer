...
- **Main Seer** ([app/api/seer/chat/route.ts](app/api/seer/chat/route.ts)): One non-streaming Groq call per user message. The Groq response includes `usage` (prompt_tokens, completion_tokens, total_tokens); the route does not currently read or log it. Typical size: system prompt + thread (last 6 messages) + new message; response `max_tokens: 500`. Rough order of magnitude: **~1k–4k tokens per exchange** (default model `openai/gpt-oss-120b` for paid, `openai/gpt-oss-20b` for free).
